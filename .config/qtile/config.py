...
# Variáveis de tema – Catppuccin Mocha
var_bg_color           = '#1e1e2e'   # background
var_active_bg_color    = '#f5c2e7'   # magenta (cor ativa/accent)
var_active_fg_color    = '#1e1e2e'   # background (contraste com magenta)
var_inactive_bg_color  = '#45475a'   # preto
var_inactive_fg_color  = '#bac2de'   # branco
var_urgent_bg_color    = '#f38ba8'   # vermelho
var_urgent_fg_color    = '#1e1e2e'   # background (contraste com vermelho)
var_section_fg_color   = '#f9e2af'   # amarelo (destaques)
var_active_color       = '#f5c2e7'   # magenta (borda ativa)
var_normal_color       = '#45475a'   # preto (borda normal)
var_border_width       = 2
var_margin             = [5, 5, 5, 5]
var_font_name          = 'JetBrainsMono Nerd Font'
...
#                         this_screen_border="#3d4555",
#                         other_current_screen_border="#81A1C1",
#                         other_screen_border="#3d4555",
#                         disable_drag=True,
#                 ),
#                 widget.Prompt(),
#                 widget.WindowName(),
...
screens = [
    Screen(
        top=bar.Bar(
            [
                # GroupBox – exibe apenas números sem bordas e sem fundo extra
                widget.GroupBox(
                    font=var_font_name,
                    fontsize=16,
                    margin_y=2,
                    ...
                    padding_y=5,
                    padding_x=3,
                    borderwidth=0,               # sem borda para remover a caixa
                    active=var_active_bg_color,
                    inactive=var_inactive_fg_color,
                    highlight_method="text",     # apenas altera a cor do texto
                    this_current_screen_border=var_active_color,
                    other_current_screen_border=var_normal_color,
                    disable_drag=True,
                    visible_groups=[str(i) for i in range(1, 6)],
                    background=var_bg_color,       # mesmo background da barra
                ),
                widget.Prompt(
                    background=var_bg_color,
                    foreground=var_inactive_fg_color,
                ),
                widget.WindowName(
                    background=var_bg_color,
                    foreground=var_inactive_fg_color,
                ),
                # Widget CPU – exibe o ícone  e o percentual de carga
                widget.CPU(
                    format='  {load_percent}%',
                    background=var_bg_color,
                    foreground=var_inactive_fg_color,
                ...
                # Widget Memory – exibe o uso em MB (MemUsed)
                widget.Memory(
                ...
                # # Widget DF – exibe o percentual de disco da partição "/"
                # widget.DF(
                #     partition="/",
                #     format='󰋊 {percentage:.0f}%',
                #     background=var_bg_color,
                #     foreground=var_inactive_fg_color,
                #     update_interval=60,
                # ),
                # Widget Volume – exibe o volume com ícone 
                widget.Volume(
                    fmt='  {}%',
                    background=var_bg_color,
                    foreground=var_inactive_fg_color,
                ...
                # Systray – exibe ícones de aplicações em segundo plano
                widget.Systray(
                    background=var_bg_color,
                    padding=5,
                ),
                # Widget Clock – exibe data e hora com ícone 
                widget.Clock(
                    format=' %Y-%m-%d %I:%M %p',
                    background=var_bg_color,
                    foreground=var_inactive_fg_color,
                ),
                # # Widget QuickExit – exibe o ícone  para sair
                # widget.QuickExit(
                #     default_text='',
                #     background=var_bg_color,
                #     foreground=var_urgent_bg_color,
                #     countdown_format=' {}',
                # ),
            ],
            34,  # Altura da barra
            ...
            border_color=[var_normal_color, var_bg_color, var_bg_color, var_bg_color],
        ),
    ),
]
